...
positivity <- dplyr::filter(ufc19, ufc19$date == max(ufc19$date))[7]
activeNow <- dplyr::filter(ufc19, ufc19$date == max(ufc19$date))[3]
totalDead <- ufc19$deaths %>% max()
quarantine <- dplyr::filter(ufc19, ufc19$date == max(ufc19$date))[8]
...
```{r}
valueBox(
  value = paste(round(positivity,digits=2)*100, "%", sep = ""),
  caption = "Testing positivity rate",
  color = "#644a8c",
